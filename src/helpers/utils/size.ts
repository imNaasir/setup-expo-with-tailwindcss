...
const tabletScaleFactor = isExtraLargeTablet
  ? 0.5
  : isLargeTablet
  ? 0.6
  : isTablet
...
export const wwp = (percentage: number) =>
  (percentage / 100) * windowWidth * tabletScaleFactor;
export const whp = (percentage: number) =>
  (percentage / 100) * windowHeight * tabletScaleFactor;
export const swp = (percentage: number) =>
  (percentage / 100) * screenWidth * tabletScaleFactor;
export const shp = (percentage: number) =>
  (percentage / 100) * screenHeight * tabletScaleFactor;
...
export const fontSize = (
  ...
  if (isTablet) {
    newSize =
      size * (scale * (isExtraLargeTablet ? 0.5 : isLargeTablet ? 0.6 : 0.7));
  }
  ...
  return Math.round(
    PixelRatio.roundToNearestPixel(
      Math.max(options.minSize, Math.min(newSize, options.maxSize))
    )
  );
};
